...
public class WeatherSubscribe {
    ...
    public static RequestBody setRequestBody(String json){
        return RequestBody.create(okhttp3.MediaType.parse("application/json;charset=UTF-8"),json);
    }

    
    public static void getHomeDate(DisposableObserver<ResponseBody> subscriber){
        Observable<ResponseBody> observable = RetrofitFactory.getInstance().getHttpApi().getHomeDate(setRequestBody("aa"));
        //                                        观察者       订阅
        RetrofitFactory.getInstance().toSubscribe(observable,subscriber);
    }
}
